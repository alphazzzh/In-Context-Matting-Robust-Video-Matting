...
class InContextDataset(Dataset):
    # divide a dataset into train set and validation set
    def __init__(self, data, crop_size=1024, phase="train",norm_type='imagenet', reference_image_name=None):
        self.phase = phase
        self.crop_size = crop_size
    ...
    def __getitem__(self, idx):
        ...
        # get reference image (fixed reference image name)
        reference_image_name = self.reference_image_name if self.reference_image_name else image_name

         # Find the dataset where the reference image is located
        reference_dataset_name = None
        for key, value in self.image_class_dict.items():
            for item in value:
                if item[0] == reference_image_name:
                    reference_dataset_name = item[1]
                    break
            if reference_dataset_name:
                break

        if not reference_dataset_name:
            raise ValueError(f"Reference image '{reference_image_name}' not found in any dataset.")

        reference_image_sample = self.get_sample(reference_image_name, reference_dataset_name)
    ...
    # Check if file exists
        if not os.path.exists(trimap_path):
            print(f"Error: File '{trimap_path}' does not exist.")
            return None  # Handle file not existing case
        else:
            # File exists, attempt to read image
            trimap = cv2.imread(trimap_path, 0)
            if trimap is None:
                print(f"Error: Failed to read image file '{trimap_path}'.")
                return None  # Handle read failure case
            else:
                # Successfully read the image, continue processing
